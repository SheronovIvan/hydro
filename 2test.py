...
def source_function(x, y, k):
    return np.cos(np.pi * k * x) * np.cos(np.pi * k * y)
...
def conjugate_gradient_laplace_solver(n, k, max_iter=None, tol=1e-6):
    h = 1.0 / n
    size = (n-1)**2  # Количество внутренних узлов
    ...
    # Функции преобразования сетка <-> вектор
    def grid_to_vec(u_grid):
        return u_grid[1:n, 1:n].flatten()
    ...
    def laplace_operator(vec):
        ...
        for i in range(1, n):
            for j in range(1, n):
                idx = (i-1)*(n-1) + (j-1)
                result[idx] = (4*u_grid[i,j] - u_grid[i-1,j] - u_grid[i+1,j] - u_grid[i,j-1] - u_grid[i,j+1])/h**2
        return result
    ...
    # Начальное приближение (можно улучшить)
    x = grid_to_vec(u)
    ...
    if max_iter is None:
        max_iter = size  # Максимум возможных итераций
    ...
    for it in range(max_iter):
        ...
        if np.sqrt(rsnew) < tol * np.linalg.norm(b):  # Относительная точность
            return vec_to_grid(x), it+1
        
        p = r + (rsnew/rsold)*p
        rsold = rsnew
    
    print(f"⚠️ Не достигнута точность за {max_iter} итераций (остаток: {np.sqrt(rsnew):.2e})")
    return vec_to_grid(x), max_iter
